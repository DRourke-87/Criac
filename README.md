...
A personal Telegram agent: send a voice note or text, and it transcribes it
...
replies in Telegram confirming what it did with a link to the Notion page.

## How it works

```
Telegram DM (voice/text)
   └─ bot.py        receives the message, gates on your Telegram user ID
...
(Whisper via Groq), `bot.py` / `main.py` (Telegram long polling),
...
1. **Telegram bot** — message [@BotFather](https://t.me/BotFather) on Telegram:
   `/newbot`, give it a name, and copy the **token** (`TELEGRAM_BOT_TOKEN`).
   Get your own user ID: message [@userinfobot](https://t.me/userinfobot) for
   your numeric ID (`TELEGRAM_ALLOWED_USER_ID`).
...
The bot uses **long polling** — no public URL or open port — so
...
> the bot must stay gated to your own Telegram user ID — don't open it up to others.
...
free web tiers, PythonAnywhere free) — a long-polling bot needs a persistent process.
